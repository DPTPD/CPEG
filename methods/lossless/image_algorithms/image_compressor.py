...
class ImageCompressor(methods.general.compressor.Compressor, abc.ABC):
    ...
    @abc.abstractmethod
    def compress_image(self, matrix: np.ndarray, pp: float, wlen: float, dist: float, mat_dtype: str,
    ...
    def decompress_image(self, input_path: str) -> (np.ndarray, float, float, float, str, int):
        """
        :return: (u8 matrix, pp, wlen, dist, mat_dtype, dtype_size)
        """
        pass
    ...
    def decompress(self, input_path: str) -> HoloSpec:
        g17ed_matrix, pp, wlen, dist, mat_dtype, dtype_size = self.decompress_image(input_path)
...
class PillowCompressor(ImageCompressor, abc.ABC):
    ...
    def decompress_image(self, input_path: str) -> (np.ndarray, float, float, float, str, int, int):
        ...
        return img, pp, wlen, dist, dtype, dtype_size
